...
while True:
    ...
    while nuevojuego == True:
        ...
        if dificultad == "dificil":
            print("\nBuscas reto? adivina la flor\n")    
            with open("C:/IBERO/juego_ahorcado/BancoDePalabrasDificil.txt","r") as archivo:
                lineas = archivo.readlines()
                palabras = [palabra.strip() for linea in lineas for palabra in linea.split()]
                palabraAleatoria = rdn.choice(palabras).strip()

        elif dificultad == "intermedio":
            print("\nBuena opcion, siempre en equilibrio, adivina el país \n")
            with open("C:/IBERO/juego_ahorcado/BancoDePalabrasIntermedio.txt","r") as archivo:
                lineas = archivo.readlines()
                palabras = [palabra.strip() for linea in lineas for palabra in linea.split()]
                palabraAleatoria = rdn.choice(palabras).strip()
    ...
    def Victoria():
        if " _" not in casillas:
            print("Felicidades ", nombre,"!!! Descubriste la palabra por completo")
            print("La palabra era", palabraAleatoria)
            #victoriasTotales = victoriasTotales + 1
            #print(f"Victorias: {victoriasTotales}, derrotas: {derrotasTotales}")
            return True
    #Si la lista de letras incorrectas alcanza 6 elementos en ella, quiere decir que perdiste
    def Perder():
        if len(letrasIncorrectas) == 6:
            print("¡Has perdido! la palabra a adivinar era: " ,palabraAleatoria)
            #derrotasTotales = derrotasTotales + 1
            #print(f"Victorias: {victoriasTotales}, derrotas: {derrotasTotales}")
            return True
    ...
    respuesta = input("¿Quieres seguir jugando? (sí/no): ").lower()
